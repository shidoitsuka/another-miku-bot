...
exports.run = async (bot, message, args) => {
  ...
  if (forbidden.some(word => message.content.toLowerCase().includes(word))) {
    return message.channel.send(
      `**${message.author.username}**, Something went wrong!`
    );
  }
  ...
  try {
    ...
    await message.channel
      .send(new Attachment(await canvas(), `canvas-${message.author.id}.jpg`))
